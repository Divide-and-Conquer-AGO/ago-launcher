...
func (configurator *Configurator) LoadConfigFile(path string, cfgStruct interface{}) *ini.File {
	...
	case ".json":
		...
		if err != nil {
			utils.Logger().Fatalf("[Config] Could not open json config file at %s: %v", absPath, err)
		}
		defer jsonFile.Close()

		byteValue, err := io.ReadAll(jsonFile)
		if err != nil {
			utils.Logger().Fatalf("[Config] Fail to json read file at %s: %v\n", absPath, err)
		}
		utils.Logger().Printf("[Config] Parsing json config file at %s to struct\n", absPath)
		err = json.Unmarshal(byteValue, cfgStruct)
		if err != nil {
			utils.Logger().Fatalf("[Config] Failed to map json config file at %s to struct: %v\n", absPath, err)
		}
		return nil
	case ".cfg":
		...
		if err != nil {
			utils.Logger().Fatalf("[Config] Fail to read ini file at %s: %v\n", absPath, err)
		}
		utils.Logger().Printf("[Config] Parsing ini config file at %s to struct\n", absPath)
		err = cfg.MapTo(cfgStruct)
		if err != nil {
			utils.Logger().Fatalf("[Config] Failed to map ini config file at %s to struct: %v\n", absPath, err)
		}
		return cfg
	}
	return nil
...
func (configurator *Configurator) WriteConfigToFile(path string, cfgData interface{}, filePtr *ini.File) {
	...
	if err != nil {
		utils.Logger().Fatalf("[Config] Failed to open config file for writing: %v\n", err)
		os.Exit(1)
	}
	...
	case ".json":
		...
		if err != nil {
			utils.Logger().Fatalf("[Config] Fail to write file: %v\n", err)
		}
	case ".cfg":
		err = filePtr.ReflectFrom(cfgData)
		if err != nil {
			utils.Logger().Fatalf("[Config] Failed to update ini file from struct: %v\n", err)
		}

		// Write the config back to the file
		_, err = filePtr.WriteTo(openedFile)
		if err != nil {
			utils.Logger().Fatalf("[Config] Failed to write config to file: %v\n", err)
		}
	}
